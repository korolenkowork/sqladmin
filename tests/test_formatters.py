    ...
    return Request(
        {
            ...
            "raw_path": b"/admin/user/details/1",
            "headers": [],
            "query_string": b"",
...
async def test_column_formatters_different_details_and_list(
...
async def test_column_formatters_list_with_inheritance_type(
    mock_request: Request,
) -> None:
    class CustomStr(str): ...
...
async def test_column_formatters_str_enum(mock_request: Request) -> None:
    custom_column_type_formatters = BASE_FORMATTERS.copy()
    ...
    assert await ProfileAdmin().get_detail_value(profile, "role", mock_request) == (
        Role.ADMIN,
        VALID_STR_ENUM_HTML,
    )

    assert await ProfileAdmin().get_detail_value(profile, "status", mock_request) == (
        None,
        Markup(""),
    )
